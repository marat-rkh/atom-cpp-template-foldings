...
export class FoldingsManager {
    ...
    foldWithPreview(bufferRange, previewText, foldingId) {
        ...
        if(softWrap && !softWrapAtPreferredLineLength) {
            return { status: FoldingStatus.SOFT_WRAP_ONLY_ENABLED };
        }
        const maxLineLength = this._getEditorMaxLineLength(softWrap, softWrapAtPreferredLineLength);
        ...
        if(check === PseudoFoldingCheck.POSSIBLE) {
            const foldingInfo = this._performFolding(bufferRange, previewText, foldingId);
    ...
    _getEditorMaxLineLength(softWrap, softWrapAtPreferredLineLength) {
        if(softWrap && softWrapAtPreferredLineLength) {
    ...
    _performFolding(bufferRange, previewText, foldingId) {
