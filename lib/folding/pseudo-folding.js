...
export default class PseudoFolding {
    ...
    constructor(textEditor, id, bufferRange, foldedAreaSize) {
        ...
        // TODO unfold everithing in the range with TextEditorUtils.unfoldBufferRange
        // (create that function first :) )
        this._fold(bufferRange, textVisualLength, foldedAreaSize);
    }

    destroy() {
        for(let r = this.bufferRange.start.row; r <= this.bufferRange.end.row; ++r) {
    ...
    _fold(bufferRange, textVisualLength, foldedAreaSize) {
        ...
        this.textEditor.setSelectedScreenRange([prefixStart, prefixEnd], { preserveFolds: true });
        this.textEditor.foldSelectedLines();
