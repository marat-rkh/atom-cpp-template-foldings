...
export class HighlightingManager {
    ...
    _removeHighlightingAtOldPosition(event) {
        ...
        foldingsAtOldRow.forEach(folding => {
            if(!folding.preview.isHidden()) {
                const range = folding.marker.getBufferRange();
                if(range.containsPoint(event.oldBufferPosition) &&
                    !range.containsPoint(event.newBufferPosition))
                {
                    logger.debug(this, `cursor left fold preview '${folding.id}'`);
        ...
        foldingsAtNewRow.forEach(folding => {
            if(!folding.preview.isHidden()) {
                ...
                if(!range.containsPoint(event.oldBufferPosition) &&
                    range.containsPoint(event.newBufferPosition))
                {
                    logger.debug(this, `cursor entered fold preview '${folding.id}'`);
                    this._addHighlighting(folding);
                }
            }
        });
