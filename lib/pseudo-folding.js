...
export default class PseudoFolding {
    ...
    constructor(textEditor, id, bufferRange, foldedAreaSize) {
        ...
        this.marker = this._fold(textVisualLength, foldedAreaSize);
    }
    ...
    _fold(textVisualLength, foldedAreaSize) {
        // fold line endings
        const bufRangeStart = this.bufferRange.start;
        const fstRow = bufRangeStart.row;
        const lastRow = this.bufferRange.end.row;
        ...
        this.textEditor.foldSelectedLines();
        return this.textEditor.markBufferRange(this.bufferRange, { invalidate: 'never' });
    }
}
